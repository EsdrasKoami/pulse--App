        ...
        <div className="group relative bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 border border-slate-100 dark:border-slate-800 transition-all duration-500 hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-2 animate-fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
            ...
            <div className="flex flex-wrap justify-center gap-1.5 mb-8">
                {(user.interests || []).slice(0, 3).map((interest, i) => (
                    <span key={i} className="px-3 py-1 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[9px] font-black rounded-lg uppercase tracking-wider">
                        {t(interest.name)}
                    </span>
                ))}
            </div>
            ...
            <button
                ...
                disabled={sending || sent || user.connection_status === 'sent_pending' || user.connection_status === 'accepted'}
            ...
            >
                {sending ? t('Envoi...') : (
                    ...
                    user.connection_status === 'accepted' ? t('Jumelé(e)') : 
